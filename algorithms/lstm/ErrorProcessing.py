...
class ErrorProcessing:
    ...
    def get_errors(self, y_test, y_hat, smoothed=True):
        """
        ...
        :param y_test (np array): array of test targets corresponding to true values to be predicted at end of each sequence
        :param y_hat (np array): predicted test values for each timestep in y_test
        :param smoothed (bool): If False, return unsmoothed errors (used for assessing quality of predictions)
        :return e (list): unsmoothed errors (residuals)
        :return e_s (list): smoothed errors (residuals)
        """
        ...
        if not len(y_hat) == len(y_test):
            raise ValueError("len(y_hat) != len(y_test), can't calculate error: %s (y_hat) , %s (y_test)"
    ...
    def process_errors(self, y_test, e_s):
        """
        ...
        :param y_test (np array): test targets corresponding to true telemetry values at each timestep t
        :param y_hat (np array): test target predictions at each timestep t
        :param e_s (list): smoothed errors (residuals) between y_test and y_hat
        :param anom (dict): contains anomaly information for a given input stream
        :param logger (obj): logging object
        :return E_seq (list of tuples): Start and end indices for each anomaloues sequence
        :return anom_scores (list): Score for each anomalous sequence
        """
        config = Config("_config_files/config_new.yaml")
        ...
        while num_windows < 0:
            ...
            if window_size == 1 and num_windows < 0:
                raise ValueError("Batch_size (%s) larger than y_test (len=%s). Adjust in config_new.yaml." % (
        ...
        for i in range(1, num_windows + 2):
            ...
            epsilon = self.find_epsilon(window_e_s, config.error_buffer)
            window_anom_indices = self.get_anomalies(window_e_s, window_y_test, epsilon, i - 1, i_anom, len(y_test))
    ...
    def find_epsilon(self, e_s, error_buffer, sd_lim=12.0):
        """
        ...
        :param e_s (array): residuals between y_test and y_hat values (smoothes using ewma)
        :param error_buffer (int): if an anomaly is detected at a point, this is the number of surrounding values
                to add the anomalous range. this promotes grouping of nearby sequences and more intuitive results
        :param sd_lim (float): The max number of standard deviations above the mean to calculate as part of the
                argmax function
        :return sd_threshold (float): the calculated anomaly threshold in number of standard deviations above the mean
        """
    ...
    def compare_to_epsilon(self, e_s, epsilon, len_y_test, inter_range, chan_std,
        ...
        :param e_s (list): smoothed errors between y_test and y_hat values
        :param epsilon (float): Threshold for errors above which an error is considered anomalous
        :param len_y_test (int): number of timesteps t in test data
        :param inter_range (tuple of floats): range between 5th and 95 percentile values of error values
        :param chan_std (float): standard deviation on test values
        :param std (float): standard deviation of smoothed errors
        :param error_buffer (int): number of values surrounding anomalous errors to be included in anomalous sequence
        :param window (int): Count of number of error windows that have been processed
        :param i_anom_full (list): list of all previously identified anomalies in test set

        :return E_seq (list of tuples): contains start and end indices of anomalous ranges
        :return i_anom (list): indices of errors that are part of an anomlous sequnce
        :return non_anom_max (float): highest smoothed error value below epsilon
        """
        config = Config("_config_files/config_new.yaml")
            ...
            if anom:
                for b in range(0, error_buffer):
                    if not x + b in i_anom and not x + b >= len(e_s) and (
                            (x + b) >= len(e_s) - config.batch_size or window == 0):
    ...
    def prune_anoms(self, E_seq, e_s, non_anom_max, i_anom):
        """
        Remove anomalies that don't meet minimum separation from the next closest anomaly or error value
        :param E_seq (list of lists): contains start and end indices of anomalous ranges
        :param e_s (list): smoothed errors between y_test and y_hat values
        :param non_anom_max (float): highest smoothed error value below epsilon
        :param i_anom (list): indices of errors that are part of an anomalous sequence
        :return i_pruned (list): remaining indices of errors that are part of an anomalous sequences after pruning procedure
        """
    ...
    def get_anomalies(self, e_s, y_test, z, window, i_anom_full, len_y_test):
        """
        ...
        :param e_s (list): smoothed errors between y_test and y_hat values
        :param y_test (np array): test targets corresponding to true telemetry values at each timestep for given window
        :param z (float): number of standard deviations above mean corresponding to epsilon
        :param window (int): number of error windows that have been evaluated
        :param i_anom_full (list): list of all previously identified anomalies in test set
        :param len_y_test (int): num total test values available in dataset

        :return i_anom (list): indices of errors that are part of an anomalous sequence
        """
        ...
        z_inv = self.find_epsilon(e_s_inv, config.error_buffer)

        epsilon = mean + (float(z) * std)
        epsilon_inv = mean + (float(z_inv) * std)

        # find sequences of anomalies greater than epsilon
        E_seq, i_anom, non_anom_max = self.compare_to_epsilon(e_s, epsilon, len_y_test,
                                                         inter_range, chan_std, std, config.error_buffer, window,
                                                         i_anom_full)

        # find sequences of anomalies using inverted error values (lower than normal errors are also anomalous)
        E_seq_inv, i_anom_inv, inv_non_anom_max = self.compare_to_epsilon(e_s_inv, epsilon_inv,
                                                                     len_y_test, inter_range, chan_std, std,
                                                                     config.error_buffer, window, i_anom_full)

        if len(E_seq) > 0:
            i_anom = self.prune_anoms(E_seq, e_s, non_anom_max, i_anom)

        if len(E_seq_inv) > 0:
            i_anom_inv = self.prune_anoms(E_seq_inv, e_s_inv, inv_non_anom_max, i_anom_inv)

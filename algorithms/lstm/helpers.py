...
def load_train_data(path_train):
    ...
    # encoding categories for the loaded sets
    # train = ch.encode_categories(train)
    # check if there are columns that need to become categorical

    # except:
    #     raise ValueError("Training data not found, on", path_train, "check train and test paths")
    print('data loaded, about to reshape')
    X_train, y_train = shape_data(train)
    return X_train, y_train


def load_test_data(chan_id, path_test):
    try:
        test = np.load(os.path.join(path_test, chan_id + ".npy"), allow_pickle=True)
    # test = ch.encode_categories(test)
...
def shape_data(arr, train=True):
    ...
    # print("range of i", range(len(arr) - config.l_s - config.n_predictions))
    # print('type config ls', type(config.l_s), 'type predictions', type(config.n_predictions))
    ...
    # print("shape of array after reshaping: X - ", np.shape(X))
    # print("shape of array after reshaping: y - ", np.shape(y))
    return X, y
